<template>
  <div class="list-wrap">
    <el-tabs @tab-click="handleTabClick" v-model="activeName" class="tabs">
          ...
          <div class="list-container">
            <div
              ...
              :class="{
                ...
                'task-notstart':
                  item.status === null || parseInt(item.status, 10) === 0,
                'task-cancel': parseInt(item.status, 10) === 4
              }"
            >
              <p class="task-title">
      ...
      <el-tab-pane label="我执行的" name="task">
        <page-init :loading="loading" :message="message">
          <div class="list-container">
            <div
              class="task-list"
...
export default {
  ...
  filters: {
    parseFlowStatus(status) {
      let st = ''
      if (status) {
        st = Vue.prototype.$constant.FLOW_STATUS.find(({ id }) => id == status)
          .name
      }
      return st
    },
    parseTaskStatus(status) {
      let st = '未开始'
      if (status) {
        st = Vue.prototype.$constant.TASK_STATUS.find(({ id }) => id == status)
          .name
      }
      return st
    }
  ...
  methods: {
    async fetchList(type) {
      this.loading = true
      this.code = 0
      const { fetchList } = this.$api.myTask
      const params = {
        rootProcessInstanceId: this.$route.params.rootProcessExec,
        type
      }
      try {
    ...
    toTaskPage() {
      ...
      }/task?myTab=0&taskTab=0&view=task`
      this.$router.push(path)
    },
