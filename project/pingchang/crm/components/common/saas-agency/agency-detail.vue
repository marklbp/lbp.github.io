<template>
  <div class="pch-detail">
    <titleField v-for="(item, index) in detail"
        ...
        <div slot='con'>
          <el-row :gutter="24" v-if="/base|guarantor/.test(item.module)||(/account/.test(item.module) &&/shop|store/.test($route.query.agencyType))">
          ...
          <el-table  v-else-if="/contact|lender|account/.test(item.module)" :fit=true :data="form[item.module]" style="width: 100%" class='pch-table' stripe>
            <el-table-column v-if="item.module!='file'" v-for="(col, i) in item.content" :prop="col.prop" :label="col.label" :key="i"></el-table-column>
            <el-table-column  :prop="col.prop" :label="col.label" :key="i" v-else-if="i == item.content.length - 1"><template slot-scope="scope"><a :href="scope.row.fileUrl" tagert="_blank">查看详细信息</a></template></el-table-column>
            <el-table-column v-else :prop="col.prop" :label="col.label" :key="i"></el-table-column>
          </el-table>
