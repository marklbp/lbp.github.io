...
AlloyLever.vueErrorHandler = function(metaData, error) {
  metaData.error = error.toString()
  console.error(error)
  reportError(JSON.stringify(metaData))
}

window.onerror = function(msg, url, line, col, error) {
    var newMsg = msg

    newMsg = errorHandle(error, newMsg)
...
function errorHandle (error, newMsg) {
...
function reportError (newMsg) {
  var ss = AlloyLever.settings
  if(ss.reportUrl) {
      var src = ss.reportUrl + (ss.reportUrl.indexOf('?')>-1?'&':'?') + '__topic__=' + ( ss.reportPrefix ?ss.reportPrefix : '') + '&' + ss.reportKey + '=' + newMsg +'&t='+new Date().getTime()
      if(ss.otherReport) {
...
function loadScript(src, callback){
    ...
    s.setAttribute('crossorigin', 'anoymous')
    s.src = src
