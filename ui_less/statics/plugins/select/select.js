+function(a) {
    ...
    function b(b) {
        ...
        a(e).each(function() {
            ...
            d.hasClass("open") && (d.trigger(b = a.Event("hide.dropdown", e)),
            b.isDefaultPrevented() || d.removeClass("open").trigger("hidden.dropdown", e))
        })
    }
    function c(b) {
    ...
    var d = ".dropdown-backdrop"
      ...
      , f = function(b) {
        a(b).on("click.dropdown", this.toggle)
      };
    f.prototype.toggle = function(d) {
        ...
        if (!e.is(".disabled, :disabled")) {
            ...
            !g) {
                ...
                if (f.trigger(d = a.Event("show.dropdown", h)),
                d.isDefaultPrevented())
                    return;
                f.toggleClass("open").trigger("shown.dropdown", h),
                e.focus()
            }
            return !1
        }
    ...
    a(document).on("click.dropdown.data-api", b).on("click.dropdown.data-api", ".dropdown form", function(a) {
        a.stopPropagation()
    }).on("click.dropdown.data-api", e, f.prototype.toggle).on("keydown.dropdown.data-api", e + ", [role=menu], [role=listbox]", f.prototype.keydown)
}(jQuery)
,function(b) {
    ...
    a.prototype = {
        ...
        createDropdown: function() {
            ...
            var e = this.options.liveSearch ? '<div class="select-searchbox"><input type="text" class="input-block-level form-control" /></div>' : "";
            var d = '<div class="select' + c + '"><button type="button" class="btn dropdown-toggle selectpicker" data-toggle="dropdown"' + g + '><span class="filter-option pull-left"></span>&nbsp;<span class="ico"></span></button><div class="dropdown-menu open">' + f + e + '<ul class="dropdown-menu inner selectpicker" role="menu"></ul></div></div>';
            return b(d)
        },
        createView: function() {
    ...
    b(document).data("keycount", 0).on("keydown", ".qb-select [data-toggle=dropdown], .qb-select [role=menu], .select-searchbox input", a.prototype.keydown).on("focusin.modal", ".qb-select [data-toggle=dropdown], .qb-select [role=menu], .select-searchbox input", function(c) {
        c.stopPropagation()
    })
}(jQuery);
