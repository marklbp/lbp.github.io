...
import {ctx, hourWidthCache, getOptions, prepareStyle, isObject, mergeDeep, isObservable, mergeDeepReactive, notEqualDeep} from './misc'
export default {
  name: 'gantt',
  components: {
    'main-view': _ => import('./main-view')
  },
  ...
  methods: {
     ...
     */
    syncScrollTop() {
      let taskListItems = this.state.refs.taskListItems
      let chartGraph = this.state.refs.chartGraph
      if (taskListItems && chartGraph && chartGraph.scrollTop !== taskListItems.scrollTop) {
        this.state.options.scroll.top = taskListItems.scrollTop = this.state.refs.chartScrollContainerVertical.scrollTop = chartGraph.scrollTop;
      }
    },
