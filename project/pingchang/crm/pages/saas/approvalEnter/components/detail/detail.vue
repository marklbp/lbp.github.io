        ...
        <div class="pch-tab pch-left-colums">
            <el-tabs class="pch-tab" v-model="currTab" >
                <el-tab-pane class="pch-tab-title" v-for="(item,index) in tabOption" :key="index" :label="item.value"
                             :name="item.name">
                    <pubDetail :blackList="blackList" v-if="index===0" :detailData="detailData" :tableData="tableData" :infoDataCustom="infoDataCustom"/>
                    ...
                    <section class="check-form-section">
                        ...
                        <div class="pch-detail-item-sm" v-if="index==='form'" v-for="(list,ind) in item.list"
                            ...
                            <div class="pch-detail-con-sm" v-if="list.name === 'remark'">
                                <el-input  v-if="lenderAgency['status']===10" v-model="form.remark" placeholder="请输入审批意见" type="textarea"
                                          :rows="2"></el-input>
                                <span v-if="lenderAgency['status']!==10">{{lenderAgency['remark']}}</span>
                            </div>
                            <div class="pch-detail-con-sm" v-if="list.name === 'status'">
                                <el-select v-if="lenderAgency['status']===10" v-model="form.status" placeholder="请选择">
                                    <el-option
                                ...
                                <span v-if="lenderAgency['status']!==10">{{lenderAgency['status']}}</span>
                            </div>
                        </div>
                    </section>
                </div>
                <div class='pch-bottom-btns' v-if="lenderAgency['status']===10">
                    <el-button class='default-sm-btn' type="primary" @click="onSubmit">完成</el-button>
                </div>
            </div>
        </div>
    </div>
