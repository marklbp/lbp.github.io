<template>
  <div>
            ...
            <el-col v-for="(col, idx) in row" :xs="24/row.length" :sm="24/row.length" :md="24/row.length" :lg="24/row.length" :xl="24/row.length" :key="idx">
              <el-form-item :label="col.label"
                ...
                <el-select v-else-if="col.type=='select'"
                  ...
                  <el-option v-for="(item, itemidx) in form.preData.baseForm[filed.name]"
                             :key="itemidx"
                             :label="item.name"
                ...
                <div v-else style="position: relative;">
                  <el-input v-model="(filed.disabled ? form.preData.baseForm : form.baseForm)[filed.name]"
                              :name="filed.name"
                              :placeholder="filed.placeholder"
                              :key="i"
                              :disabled="filed.disabled"
                  ...
                  <span style="position:absolute;right:0;top:0" v-if="filed.attach && filed.attach.type=='button'"><el-button @click="getAdmissionDetailByCard">{{filed.attach.text}}</el-button></span>
                </div>
            ...
            <el-col v-for="(col, idx) in row" :xs="24/row.length" :sm="24/row.length" :md="24/row.length" :lg="24/row.length" :xl="24/row.length" :key="idx">
              <el-form-item :label="col.label"
                ...
                <el-select v-else
                           ...
                           :class="{'input': true, 'is-danger': errors.has(filed.name) }"
                           :name="filed.name" @change="attachSelect($event, filed.name, col.attach)">
                  <el-option v-for="(item, itemidx) in filed.options"
                             :key="itemidx"
                             :label="item.name"
                             :value="item.value"></el-option>
                </el-select>
                ...
                <template slot-scope="scope">
                  <div :class="{ 'vee-control': true }">
                    ...
                    <el-select v-else
                      ...
                      <el-option v-for="(item, itemidx) in edit.contactForm.contactTypes"
                                 :key="itemidx"
                                 :label="item.name"
                                 :value="item.value"></el-option>
                  </el-select>
    ...
    <el-dialog title="添加账号" :visible.sync="dialogAccountForm.visible" :center="true" width="40%">
