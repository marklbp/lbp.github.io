...
function router(req,res,pathname){
  var staticFilePath = __dir__ + pathname;
...
function dealWithStatic(req, res, staticFilePath, pathname){
  fs.exists(staticFilePath, function(exists){
    if(!exists){
      ...
      log(req, new Date().toLocaleString(), 'GET', staticFilePath.split(__dir__)[1], 404, 'Not Found')
      res.end();
    }else{
  ...
  fs.exists(staticFilePath, function(exists){
    if (!exists) {
      ...
      if (req.headers["if-modified-since"] && lastModified == req.headers["if-modified-since"]) {
        ...
        log(req, new Date().toLocaleString(), 'GET', staticFilePath.split(__dir__)[1], 304, 'Not Modified')
        res.end()
        return;
      }
      fs.readFile(staticFilePath, "binary", function(err, file) {
        if (err) {
          ...
          log(req, new Date().toLocaleString(), 'GET', staticFilePath.split(__dir__)[1], 500, err)
          res.end(err.toString());
        } else {
          res.setHeader("Last-Modified", lastModified);
          res.writeHead(200, {'Content-Type': mineType + '; charset=UTF-8'});
          log(req, new Date().toLocaleString(), 'GET', staticFilePath.split(__dir__)[1], 200)
...
var actions = {
    ...
    redirect: function (req, res, url, old) {
      ...
      res.write(fs.readFileSync(path.join(__dir__,"/index.html")), 'binary');
      res.end()
    }
  }
...
function watch () {
  var fsWatcher = fs.watch(__dir__, {
...
function extendStringColors (){
  var prefix = '\33[{n}m'
  var appendfix = '\33[{n}m'
  var dict = {
    reset: [0, 0],
    bold: [1, 22],
    dim: [2, 22],
  ...
  Object.keys(dict).forEach(key => {
    Object.defineProperty(String.prototype, key, {
      get () {
        var px = prefix.replace('{n}', dict[key][0])
        var ax = appendfix.replace('{n}', dict[key][1])
        return px + this + ax
      }
    })
  })
