...
<script>
    ...
    export default {
        ...
        methods: {
            eventBus(event, item) {
                if(!this[event])return this.$message.info("该功能开发中，敬请期待！")
                this[event](item)
            },
            initData() {
            ...
            getTableData() {
                ...
                api.getUserList_ext(this.params).then((res) => {
                    if(res.data){
            ...
            getEmployeeList() {
                api.getUserList_ext({ownerId:this.$route.query.ownerId}).then((res) => {
                    this.employeeList = res.data.list
                })
            },
            searchData(data) {
                ...
                api.getAllRoles_ext(obj).then((res) => {
                    let {data} = res
                    if (!data) {
                        this.FLAG = false;
                        return false
                    ...
                    api.getRolePrivilege_ext(resultParames).then((res) => {
                        this.privilege = [];
                        this.defaultKeys = [];
            ...
            add() {
                api.getRoleList_ext({ownerId:this.ownerId}).then((res) => {
            ...
            copySut() {
                ...
                api.copyUserPrivilege_ext(obj).then((res) => {
                    if (res.code === 200) {
                        this.$message({
                            message: '复制员工权限成功！',
                            type: 'success'
            ...
            handleSearchUsername() {
                ...
                api.getInfoByUserName_ext({userName:this.searchUsername,ownerId:this.ownerId}).then((res) => {
                    if (res.data) {
            ...
            searchName() {
                ...
                api.getRoleList_ext({name: this.searchname,ownerId:this.ownerId}).then((res) => {
                    if (res.data) {
            ...
            qr() {
                ...
                api.addUserRoles_ext(objp).then((res) => {
                    if (res.code === 200) {
                        this.$message({
                            message: '添加角色成功！',
                            type: 'success'
