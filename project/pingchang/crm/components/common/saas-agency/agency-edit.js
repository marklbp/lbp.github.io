//saas 展厅、门店、集团、经销商、公司的准入申请(sass/admission/add)
// saas 展厅、门店的编辑(sass/admission/edit)
// saas 集团、公司、经销商的编辑(sass/admission/ssedit)
...
export default {
          ...
          ,form: {
              preData:{
                  ...
                  baseForm:{
                      ...
                      ,pca: '' //省市区
                      ,cardType: [...DefaultOpts.company_type]
                  }
                  ,outerForm: []
                  ,accountForm: []
                  ,contactForm: {
    ...
    methods: {
      arr2map(arr, keyVal){
          ...
          if('object' == typeof keyVal)map = {...map, ...keyVal}
          return map
      }
      ,pow(x){
        return Math.pow(2, x)
      ...
      ,initFormCallback(a, res){
          ...
          this.form.preData[a.form] = (()=>{
              ...
              let form = 'function' == typeof a.transform && a.transform(data, this.form) || data;
              return form;
          })()
      }
      ,parseAddress(add, p, c, a){
          //console.log(p,c,a, parseAddress(p,c,a))
          try{
      ...
      ,onSave(strType){
        ...
        this.$validator.validateAll(params).then(ok => {
          ...
          API[mod][method](params).then(res => {
            this.isClickFlag = false;//点击保存 调用保存接口成功后 isChange改为false 防止连续调用保存接口
            // this.saved = !res.error;
            if(this.currentForm==='attachForm'){//每次成功保存附件后重新给起赋值
      ...
      ,attachSelect(value, name, attach) {
        ...
        attach.forEach(a => {
          if (name === 'parentId' && a === 'lenderId') return;
      ...
      ,getGroupName(agencyId){
          ...
          .then(res=>{
              ...
              this.form.baseForm.groupName = res.success.ccgName;
          })
          .catch(err=>err)
      }
      ...
      ,getAdmissionDetailByCard(){
        let cardType = this.form.baseForm.cardType;
        if(!cardType)return;
        if(this.loading)return;
        this.loading = true;
        let cardNo = this.form.baseForm.cardNo;
        let agencyType = this.agencyTypesNames[this.$route.query.agencyType]
        API.agency.getAdmissionDetailByCard({cardNo, cardType, agencyType}).then(res=>{
            if(!res || res.error || !res.success || 'object' != typeof res.success)return;
            let data = res.success;
            this.edit.baseForm.fieldsDisabled.forEach(k=>{
                if(k == 'address'){
                    let pid = data.agencyDetail.province;
                    let cid = data.agencyDetail.city;
                    let aid = data.agencyDetail.area;
                    this.form.preData.baseForm[k] = this.parseAddress(data.agencyDetail[k], pid, cid, aid)
                    return;
                }
                if(k == 'regCapital'){
                  this.form.preData.baseForm[k] = this.regCapitalHandler(data.agencyDetail[k], data.agencyDetail.regCapitalTypeName)
                  return;
                }
                this.form.preData.baseForm[k] = data.agencyDetail[k]
            })
            this.form.preData.baseForm.contactsList = data.contactsList
            this.form.baseForm.agencyId = res.success.agencyDetail.id;
        }).catch(err=>err).finally(r=>this.loading = false)
      }
      ,dialogType(param) {
      ...
      onCheck() {
        ...
        API.agency.postCheckSubmitInfo({type,id}).then((res)=>{
          this.isClickFlag = false;
          let url = "/saas/admission/"+this.$route.query.agencyType+"?buttonId="+this.$route.query.buttonId;
