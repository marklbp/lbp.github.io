...
function dealWithStatic(req, res, staticFilePath, pathname){
  fs.exists(staticFilePath, function(exists){
    if(!exists){
      ...
      log(req, new Date().toLocaleString(), 'GET', staticFilePath.split(__dirname)[1], 404, 'Not Found')
      res.end();
    }else{
  ...
  fs.exists(staticFilePath, function(exists){
    if (!exists) {
      ...
      if (req.headers["if-modified-since"] && lastModified == req.headers["if-modified-since"]) {
        ...
        log(req, new Date().toLocaleString(), 'GET', staticFilePath.split(__dirname)[1], 304, 'Not Modified')
        res.end()
        return;
      }
      fs.readFile(staticFilePath, "binary", function(err, file) {
        if (err) {
          ...
          log(req, new Date().toLocaleString(), 'GET', staticFilePath.split(__dirname)[1], 200)
