...
Object.assign(Scroller.prototype, {
  ...
  scrollTo: function scrollTo(left, top, animate) {
    ...
    if (this._isDecelerating) {
      _animate2.stop(this._isDecelerating);
  ...
  doTouchStart: function doTouchStart(touches, timeStamp) {
    ...
    if (this._isAnimating) {
      _animate2.stop(this._isAnimating);
  ...
  _publish: function _publish(left, top) {
    var _this = this;
    ...
    if (animate && this.options.animating) {
      ...
      var doAnimation = function doAnimation() {
        ...
        _this._isAnimating = _animate2.start(step, verify, completed, _this.options.animationDuration, wasAnimating ? _animate2.easeOutCubic : _animate2.easeInOutCubic);
      };

      if (this.options.inRequestAnimationFrame) {
        _animate2.requestAnimationFrame(function () {
          doAnimation();
        });
      } else {
        doAnimation();
  ...
  _computeScrollMax: function _computeScrollMax(zoomLevel) {
    if (zoomLevel == null) {
  ...
  _startDeceleration: function _startDeceleration(timeStamp) {
    var _this2 = this;
    ...
    this._isDecelerating = _animate2.start(step, verify, completed);
  }, 
  _stepThroughDeceleration: function _stepThroughDeceleration(render) {
    //
...
_index.extend(Scroller.prototype, members);