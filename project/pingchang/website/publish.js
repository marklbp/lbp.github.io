...
R.question("请输入本次打包的仓库地址：", repo=>{
	proRepo = repo;
	if(proRepo.length <= 0){
		readlineErr = true;
		console.log('输入仓库无效'.redc)
		return R.close()
	}
	R.question("请输入本次打包分支名（可供选项：ft1|ft2|ft3|master）：", (branch)=>{
		if(/ft[1-3]|master/.test(branch)){
			...
			readlineErr = true;
			console.log('输入打包分支名有误，可供选择分支名：ft1|ft2|ft3|master'.redc);
			R.close();
		}
	})
})
...
function packageFn(branch = 'ft1', comment){
	...
	exec(cmd,function (error, stdout, stderr){
		if(error)return console.error(error.redc);
		console.log("发布完成".greenc);
		console.log("------------------------------------------------------------------");
		console.log(("输出信息"+ stdout).greenc)
	})
}