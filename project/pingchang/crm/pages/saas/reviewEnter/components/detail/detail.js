...
export default {
    ...
    data() {
        ...
        if(this.$route.path.indexOf('group') < 0){
            data.tableData.push(accountInfo)
        }
        return data;
    },
    created() {
        ...
        //if(this.applyNumber)this.getTrackApproval({applyNumber:this.applyNumber})
    },
    computed: {
        ...mapState({
    ...
    methods: {
        ...
        onSubmit() {
            if(!this.form.remark || !this.form.status){
                return this.$message.warning("请输入审查或审批意见并选择对应的结果")
            }
            this.form.applyNumber = this.applyNumber;
            if(this.onSubmit.loading)return;
            this.onSubmit.loading = true;
            api[this.netWork.api](this.form)
            .then((res) => {
            ...
            .finally(()=>delete this.onSubmit.loading)
        },
        renderStatus(status){
