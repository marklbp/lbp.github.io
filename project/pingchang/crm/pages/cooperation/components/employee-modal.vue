                    ...
                    <el-row :gutter="24">
                        ...
                        <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
                            <el-form-item label="邮箱">
                                <el-input v-validate="'email'" :class="{'input': true,'is-danger': errors.has('email') }" name="email" v-model='userList.email' placeholder="邮箱" class="input-with-select">
                                </el-input>
                        ...
                        <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
                            <el-form-item label="机构" :class="{ 'vee-control': true }">
                                <el-cascader @blur="changeBlur" expand-trigger="hover" change-on-select :options="orgData"
                                             :show-all-levels="false"
                                             v-model="departmentId" @change="selectParentId"
                                             :props="props"></el-cascader>
                                <span v-show="errors.has('userList.organizationId')" class="help is-danger">{{ errors.first('userList.organizationId') }}</span>
                            </el-form-item>
                        </el-col>
                        <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
                            <el-form-item label="角色" :class="{ 'vee-control': true }">
...
<script>
    ...
    export default {
        ...
        computed: mapState({
            ...
            roleData: state => state.ext_org.roleList,
        }),
        methods: {
            ...mapActions(['getRoleList_ext']),
            ...
            submit() {
                this.$validator.validateAll().then((result) => {
                    if (result) {
                        ...
                        this.userList.ownerId = this.ownerId
                        //console.log(this.ownerId)
                        if (this.FLAG) {
                            delete this.userList.password;
                            // 修改
                            api.putUser_ext(this.userList).then((res) => {
                                if (res.code === 200) {
                                    this.$message({
                                        message: '修改成功～',
                                        type: 'success'
                            ...
                            this.userList.roleids = this.roleids.toString()
                            api.addUser_ext(this.userList).then((res) => {
                                if (res.code === 200) {
                                    this.$message({
                                        message: '新增成功',
                                        type: 'success'
