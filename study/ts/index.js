(function(context){
  return context.extend(context,{
    ...
    init: function (){
      this.updateCache().initEvents().renderContent(this.config.elements.aside, 'SUMMARY').updateContent()
    ...
    initEvents: function (){
      ...
      $(document).on(click, 'a.section-link', function(e){
        ...
        var to
        if (list.length <= 0) {
          list = [].slice.call(that.config.elements.aside.find("a").map(function(){return this.href.slice(0, -3).replace(window.location.protocol + '//' + window.location.host + window.location.pathname.slice(0, -10), '')}))
        }
      ...
      }).on(click, '#aside_content a', function () {
        window.location.hash = this.href.slice(0, -3).replace(window.location.protocol + '//' + window.location.host + window.location.pathname.slice(0, -10), '')
        return false
      });
      this.config.elements.content.parent().on("scroll", function(){
        var sT = $(this).scrollTop()
        that.config.elements.backTop.css("display", sT > 0 ? 'inline-block' : 'none')
        var sH = that.config.elements.content.height()
    ...
    getFileName: function () {
      return location.hash.split('#')[1] || 'getting-started'
    },
