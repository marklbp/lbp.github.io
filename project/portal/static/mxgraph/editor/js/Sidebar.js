...
Object.assign(Sidebar.prototype, {
  ...
  init: function(xml) {
    ...
    xml = window.portalShapes.replace(/<shape[^>]*?name="flow(?:a|b|c)"[^>]*?>(.|\n)*?<\/shape>/g, '')
    this.addStencilPalette('portal', '流程元件', dir + '/portal.xml', ';whiteSpace=wrap;html=1;fillColor=#EDEDED;strokeColor=#999;strokeWidth=0', null, null, null, null, null, xml);
  },
  /**
   * Adds the given stencil palette.
   */
  addStencilPalette: function(id, title, stencilFile, style, ignore, onInit, scale, tags, customFns, xml) {
    ...
    if (this.addStencilsToIndex) {
      ...
      var callback = mxUtils.bind(this, function(packageName, stencilName, displayName, w, h) {
      ...
      if (!xml) {
        mxStencilRegistry.loadStencilSet(stencilFile, callback, true, true);
      } else {
        this.installShapesByXml(stencilFile, xml, callback)
      }

      this.addPaletteFunctions(id, title, xml ? true : false, fns);
    } else {
  ...
  installShapesByXml: function(stencilFile, xml, postStencilLoad) {
    var doc = mxUtils.parseXml(xml)
    mxStencilRegistry.packages[stencilFile] = doc
    mxStencilRegistry.parseStencilSet(doc.documentElement, postStencilLoad, true);
  },
  /**
   * Creates the array of tags for the given stencil. Duplicates are allowed and will be filtered out later.
   */
  getTagsForStencil: function(packageName, stencilName, moreTags) {
  ...
  createVertexTemplateEntry: function(style, width, height, value, title, showLabel, showTitle, tags) {
    ...
    showLabel = false
    // 临时解决方案来给流程部分元素添加value
    if (style.includes('mxgraph.portal')) {
      if (title === 'task') {
        value = '新建任务'
   ...
   */
  addPaletteFunctions: function(id, title, expanded, fns) {
   ...
   * Adds the given palette.
   */
  addPalette: function(id, title, expanded, onInit) {
   ...
   */
  addFoldingHandler: function(title, content, funct) {
   ...
   * Creates a drop handler for inserting the given cells.
   */
  createVertexTemplate: function(style, width, height, value, title, showLabel, showTitle, allowCellsInserted) {
   ...
   * Creates a drop handler for inserting the given cells.
   */
  createVertexTemplateFromCells: function(cells, width, height, title, showLabel, showTitle, allowCellsInserted) {
   ...
   */
  createItem: function(cells, title, showLabel, showTitle, width, height, allowCellsInserted) {
    var elt = document.createElement('a');
    elt.className = 'geItem';
    // elt.style.overflow = 'hidden';
    var newHeight = title ? 60 : this.thumbHeight;
    var border = (mxClient.IS_QUIRKS) ? 8 + 2 * this.thumbPadding : 2 * this.thumbBorder;
    elt.style.width = (this.thumbWidth + border) + 'px';
    elt.style.height = (newHeight + border) + 'px';
    elt.style.padding = this.thumbPadding + 'px';
    elt.style.marginBottom = '15px';
    this.createItemContent(elt, title, cells);
    if (mxClient.IS_IE6) {
  ...
  createItemContent(elt, title, cells) {
    var item = document.createElement('div');
    item.className = 'portal-shape portal-' + cells[0].flow
    elt.appendChild(item)
    var titleEl = document.createElement('span');
    titleEl.innerText = this.covertTitleToZh(title);
    titleEl.className = 'portal-' + cells[0].flow + '-title'
    title && elt.appendChild(titleEl)
  },
  // 流程英文名转中文
  covertTitleToZh: function  (text) {
    switch (text) {
      case 'task':
        return '任务'
      case 'start':
   ...
   */
  createThumb: function(cells, width, height, parent, title, showLabel, showTitle, realWidth, realHeight) {
    ...
    node.style.top = this.thumbBorder + (title ? -26 + this.thumbBorder : 0) + 'px';
   ...
   */
  createDragSource: function(elt, dropHandler, preview, cells, bounds) {
   ...
   * Creates a drop handler for inserting the given cells.
   */
  createDropHandler: function(cells, allowSplit, allowCellsInserted, bounds) {
   ...
   */
  addClickHandler: function(elt, ds, cells) {
   ...
   * Adds a handler for inserting the cell with a single click.
   */
  itemClicked: function(cells, ds, evt, elt) {
    var graph = this.editorUi.editor.graph;
   ...
   * Creates a drag source for the given element.
   */
  dropAndConnect: function(source, targets, direction, dropCellIndex, evt) {
   ...
   * Creates a drop handler for inserting the given cells.
   */
  updateShapes: function(source, targets) {
    var graph = this.editorUi.editor.graph;
   ...
   * Creates a drag source for the given element.
   */
  getDropAndConnectGeometry: function(source, target, direction, targets) {
    var graph = this.editorUi.editor.graph;
    var view = graph.view;
   ...
   * Adds all palettes to the sidebar.
   */
  showTooltip: function(elt, cells, w, h, title, showLabel) {
      ...
      if (this.currentElt != elt) {
        ...
        var show = mxUtils.bind(this, function() {
          ...
          if (this.tooltipTitles && title != null && title.length > 0) {
            ...
            mxUtils.write(this.tooltipTitle, this.covertTitleToZh(title));
   ...
   */
  thumbPadding: (document.documentMode >= 5) ? 2 : 3,
