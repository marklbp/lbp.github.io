<template>
  <div class="list-wrap">
    <el-tabs @tab-click="handleTabClick" v-model="activeName" class="tabs">
      <el-tab-pane label="店铺流程" name="process">
        <page-init :loading="loading" :message="message">
          <div class="list-container">
            <div
              ...
              :key="i"
              @click="clickProcessHandler(item)"
              :class="{
                'process-delay': item.isOverdue,
                'process-finish': parseInt(item.status, 10) === 2,
                'process-paush':
                  parseInt(item.status, 10) === 4 ||
                  parseInt(item.status, 10) === 3,
                'process-notstart': parseInt(item.status, 10) === 0
              }"
            >
              <div class="process-title">
                <overflow-tip
                  ...
                  class="title-words"
                ></overflow-tip>
                <p v-if="parseInt(item.status, 10) === 1">
                  <!-- 完成度: {{ item.rateProgress }} -->
                  进行中
                </p>
                <p v-else>{{ item.status | parseFlowStatus }}</p>
              </div>
          ...
          <p class="btn">
            <a href="javascript:;" @click="toProcessPage">全部 ></a>
          </p>
        </page-init>
      </el-tab-pane>
      <el-tab-pane label="店铺任务" name="task">
        <page-init :loading="loading" :message="message">
          <div class="list-container">
            <div
              class="task-list"
            ...
            >
              ...
              <peoples
                class="task-joiner"
                :list="[
                  { name: item.assigneeName || '', color: item.assigneeColor }
                ]"
              ></peoples>
            </div>
          </div>
...
export default {
  ...
  watch: {
    ...
    $route(newV, oldV) {
      if (newV.params.storeId !== oldV.params.storeId) {
  ...
  methods: {
    ...
    clickProcessHandler(item) {
      const path = `/${this.$route.params.storeId}/flow-detail/${
        item.rootProcessExec
      }`
    ...
    clickTaskHandler(item) {
      ...
      const { storeId: groupId } = this.$route.params
    ...
    toTaskPage() {
      const path = `/${this.$route.params.storeId}/task?taskTab=2&view=task`
      this.$router.push(path)
    },
