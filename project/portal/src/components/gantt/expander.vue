...
<template>
  <div :class="getClassPrefix() + '-wrapper'" :style="{ ...root.style[getClassPrefix(false) + '-wrapper'], ...style }">
    <svg
      ...
      @click="toggle"
    >
      <path :x="border" :y="border" :d="`M ${border + options.size/4},${border + options.size/4} L ${border + options.size/2},${border + options.size/2} L ${border + options.size/4},${border + options.size*3/4}`" v-if="collapsed" style="fill: transparent; stroke: #999;"/>
      <path :x="border" :y="border" :d="`M ${border + options.size/4},${border + options.size/4} L ${border + options.size/2},${border + options.size/2} L ${border + options.size*3/4},${border + options.size/4}`" v-else style="fill: transparent; stroke: #999;"/>
      <!-- <rect
