                ...
                <div class="pch-assembly-table">
                    ...
                    <article class="pch-assembly-right">
                        ...
                        <div class="patb">
                            ...
                            <!-- <el-button class='default-sm-btn' type="primary" size="small" @click="addTree">导入</el-button> -->
                        </div>
                        <div v-if="FLAG">
                            ...
                            <elTablePage :params="tabparams" url="/ext/user/getInfoListByDepartmentId"
                                         ref="tablePagination">
                                ...
                                <el-table-column prop="userName" width="200" label="登录名">
                                    <template slot-scope="scope">
                                        <span>{{ scope.row.userName || "--" }}</span>
                                    </template>
                                </el-table-column>
                                <el-table-column prop="sex" width="200" label="性别">
                                    <template slot-scope="scope">
                                        <span>{{ scope.row.sex === 1 ? '男' : '女'  }}</span>
                                    </template>
                                </el-table-column>
                                <el-table-column prop="departmentName" width="200" label="机构">
                                    <template slot-scope="scope">
                                        <span>{{ scope.row.departmentName || "--" }}</span>
                                    </template>
                                </el-table-column>
                                <el-table-column prop="status" width="250" label="是否有效">
                                    <template slot-scope="scope">
        ...
        methods: {
            ...
            initData() {
                ...
                api.getOrgList_ext(this.params).then((res) => {
                    if (res.data) {
                        ...
                        this.tabparams.departmentId = data[0].id
                        this.userList.departmentId = [data[0].id]
                    }
            ...
            reset(row){
                this.$prompt('请输入新的密码', '提示', {
                    ...
                    api.resetPassword_ext({newPassword:value,userId:row.id,ownerId:this.ownerId}).then((res)=>{
                        if(res&&res.code==200){
                            this.$message({
                                type: 'success',
            ...
            delete(row) {
                this.$confirm('此操作将永久删除该员工, 是否继续?', '提示', {
                    ...
                    api.delUser_ext({id: row.id, ownerId: this.ownerId}).then((res) => {
                        if (res.code == 200) {
                            this.$message({
                                type: 'success',
                                message: '删除成功!'
